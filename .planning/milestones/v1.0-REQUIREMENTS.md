...
- [x] **EXPORT-01**: All 18 DuckDB tables exported as parquet files to S3 bucket
- [x] **EXPORT-02**: Table-level comments from source DuckDB preserved through export
- [x] **EXPORT-03**: Column-level comments from source DuckDB preserved through export
- [x] **EXPORT-04**: WKB_BLOB geometry columns converted to native geometry types during export
- [x] **EXPORT-05**: Large tables (19M+ row EPCs) exported without OOM or timeout failures

### Pins Access

- [x] **PINS-01**: Analyst can list available datasets from R using `pin_list(board)`
- [x] **PINS-02**: Analyst can read any table from R using `pin_read(board, "table_name")`
- [x] **PINS-03**: Analyst can list available datasets from Python using pins
- [x] **PINS-04**: Analyst can read any table from Python using pins
- [x] **PINS-05**: Pin metadata includes table description and column descriptions
...
- [x] **INFRA-01**: Read-only IAM policy created for analyst AWS users
- [x] **INFRA-02**: Documentation for analysts on how to configure AWS credentials
...
- [x] **CAT-01**: Queryable table-of-tables listing all available datasets with descriptions
- [x] **CAT-02**: Each dataset listing includes column names, types, and descriptions
- [x] **CAT-03**: Each dataset listing includes row count and last updated date
